...
$orderBrand = (string) ($_GET['brand'] ?? $_POST['brand'] ?? '') === 'vans' ? 'vans' : 'praise';
$cartKey = $mode === 'single' ? 'pdd_direct_order' : ($orderBrand === 'vans' ? 'pdd_vans_cart' : 'pdd_cart');
...
?>
<!doctype html>
<html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><meta name="theme-color" content="<?= $orderBrand === 'vans' ? '#e2231a' : '#030604' ?>" /><title>Checkout — <?= $orderBrand === 'vans' ? 'Vans' : 'Praise Did Dat' ?></title><link rel="stylesheet" href="styles.css" /></head>
<body class="customer-shop customer-subpage<?= $orderBrand === 'vans' ? ' vans-cart-page' : '' ?>">
  <header class="customer-header"><a class="customer-brand" href="<?= $orderBrand === 'vans' ? 'vans-shop.php' : 'shop.php' ?>"><?= $orderBrand === 'vans' ? '<span class="customer-brand-mark">V</span><span>VANS</span>' : '<span class="customer-brand-mark">p.</span><span>praise did <b>dat</b><sup>™</sup></span>' ?></a><nav aria-label="Shop navigation"><a href="shop.php#products">Products</a><a href="shop.php#services">Services</a><a href="orders.php">My orders</a></nav><div class="customer-header-actions"><a class="customer-cart-link" href="cart.php<?= $orderBrand === 'vans' ? '?brand=vans' : '' ?>">Cart <b><?= array_sum(is_array($_SESSION[$orderBrand === 'vans' ? 'pdd_vans_cart' : 'pdd_cart'] ?? null) ? $_SESSION[$orderBrand === 'vans' ? 'pdd_vans_cart' : 'pdd_cart'] : []) ?></b></a><a href="signout.php">Sign out</a></div></header>
  <main class="customer-page-main"><p class="customer-kicker">ALMOST YOURS</p><h1>Review your <em>order.</em></h1><p class="customer-page-lede">We’ll double-check availability and get in touch about payment and delivery.</p>
    ...
    <?php if ($lines): ?><div class="customer-checkout-layout"><section class="customer-checkout-items"><h2>Your order</h2><?php foreach ($lines as $line): ?><article class="customer-checkout-line"><span><?= (int) $line['quantity'] ?> ×</span><div><strong><?= pdd_customer_escape($line['name']) ?></strong><small><?= $line['price'] === null ? 'Quote to be confirmed' : pdd_customer_escape($currency) . ' ' . number_format((float) $line['unit_price'], 2) ?></small></div><b><?= $line['price'] === null ? 'Quote' : pdd_customer_escape($currency) . ' ' . number_format((float) $line['line_total'], 2) ?></b></article><?php endforeach; ?>
      <form class="customer-checkout-form" method="post" action="checkout.php?mode=<?= pdd_customer_escape($mode) ?><?= $orderBrand === 'vans' ? '&brand=vans' : '' ?>"><input type="hidden" name="csrf_token" value="<?= pdd_customer_escape($csrf) ?>" /><input type="hidden" name="action" value="place_order" /><input type="hidden" name="mode" value="<?= pdd_customer_escape($mode) ?>" /><input type="hidden" name="brand" value="<?= $orderBrand ?>" />
      ...
      </form></section><aside class="customer-checkout-side"><p class="customer-kicker">ORDERING WITH <?= $orderBrand === 'vans' ? 'VANS' : 'PRAISE DID DAT' ?></p><h2>Made with care.</h2><p>Your order will appear under My Orders as soon as you place it. We’ll update its progress there.</p><a class="customer-text-link" href="<?= $mode === 'single' ? ($orderBrand === 'vans' ? 'vans-shop.php' : 'shop.php') : 'cart.php' . ($orderBrand === 'vans' ? '?brand=vans' : '') ?>">Back to <?= $mode === 'single' ? 'shop' : 'cart' ?></a></aside></div><?php else: ?><div class="customer-empty-panel"><p>Your selected items are unavailable.</p><a class="customer-button" href="cart.php<?= $orderBrand === 'vans' ? '?brand=vans' : '' ?>">Return to cart <span>↗</span></a></div><?php endif; ?>
