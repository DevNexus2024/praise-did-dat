...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!pdd_verify_csrf($_POST['csrf_token'] ?? null)) {
        ...
        if (in_array($action, ['add_to_cart', 'buy_now'], true)) {
            ...
            $statement = $db->prepare('SELECT id, kind FROM catalog_items WHERE id = :id AND active = 1 LIMIT 1');
            ...
            if (!$item) {
                ...
                if ($action === 'buy_now') {
                    ...
                    header('Location: checkout.php?mode=single', true, 303);
                    exit;
                }
                $cart[(int) $item['id']] = min(99, (int) ($cart[(int) $item['id']] ?? 0) + $quantity);
                $_SESSION['pdd_cart'] = $cart;
            ...
            if ($quantity <= 0) unset($cart[$itemId]);
                ...
                $statement = $db->prepare('SELECT kind FROM catalog_items WHERE id = :id AND active = 1 LIMIT 1');
                $statement->execute(['id' => $itemId]);
                $kind = $statement->fetchColumn();
                if (!$kind) {
                    unset($cart[$itemId]);
                    $notice = 'An unavailable item was removed from your cart.';
                } else {
                    $cart[$itemId] = $kind === 'service' ? 1 : $quantity;
                }
            }
            $_SESSION['pdd_cart'] = $cart;
...
<body class="customer-shop customer-subpage">
  ...
  <main class="customer-page-main"><p class="customer-kicker">YOUR PICKS</p><h1>Your <em>cart.</em></h1><p class="customer-page-lede">A little look at the good things you picked.</p>
    ...
    <?php if (!$cartRows): ?><div class="customer-empty-panel"><p>Your cart is waiting for a little joy.</p><a class="customer-button" href="shop.php">Explore the shop <span>↗</span></a></div><?php else: ?>
      <div class="customer-cart-layout"><section class="customer-cart-items" aria-label="Cart items">
        <?php foreach ($cartRows as $item): ?><article class="customer-cart-row<?= (int) $item['active'] !== 1 ? ' is-unavailable' : '' ?>">
          ...
          <form method="post" action="cart.php"><input type="hidden" name="csrf_token" value="<?= pdd_customer_escape($csrf) ?>" /><input type="hidden" name="action" value="remove" /><input type="hidden" name="item_id" value="<?= (int) $item['id'] ?>" /><button class="customer-remove" type="submit" aria-label="Remove <?= pdd_customer_escape($item['name']) ?>">Remove</button></form>
        </article><?php endforeach; ?>
        <form method="post" action="cart.php" class="customer-clear-cart"><input type="hidden" name="csrf_token" value="<?= pdd_customer_escape($csrf) ?>" /><input type="hidden" name="action" value="clear" /><button type="submit">Clear cart</button></form>
      </section><aside class="customer-cart-summary"><p class="customer-kicker">ORDER SUMMARY</p><h2>Looking good.</h2><div><span>Known item total</span><strong>SZL <?= number_format($total, 2) ?></strong></div><small>Any service quote will be confirmed after you send the request. Payment is arranged after the order is reviewed.</small><a class="customer-button" href="checkout.php?mode=cart">Continue to checkout <span>↗</span></a><a class="customer-text-link" href="shop.php">Keep browsing</a></aside></div>
    <?php endif; ?>
