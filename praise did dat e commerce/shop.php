...
$items = $db->query("SELECT id, kind, name, description, price, currency, image_url FROM catalog_items WHERE active = 1 ORDER BY kind ASC, created_at DESC, id DESC")->fetchAll();
...
<body class="customer-shop">
  <header class="customer-header">
    ...
    <div class="customer-header-actions"><a class="customer-cart-link" href="cart.php">Cart <b><?= (int) $cartCount ?></b></a><a href="signout.php">Sign out</a></div>
  </header>
  <main>
    <section class="customer-welcome"><div><p class="customer-kicker">YOUR PRAISE DID DAT SHOP</p><h1>Welcome in,<br /><em><?= pdd_customer_escape($customer['first_name']) ?>.</em></h1><p>Find a little joy for yourself, or let’s make one of your ideas real.</p><div class="customer-welcome-links"><a class="customer-button" href="#products">Explore products <span>↓</span></a><a class="customer-text-link" href="orders.php">Track my orders <span>↗</span></a></div></div><div class="customer-welcome-orbit" aria-hidden="true"><span>p.</span><i></i><b></b></div></section>
    <?php if ($flash !== ''): ?><p class="customer-flash" role="status"><?= pdd_customer_escape($flash) ?></p><?php endif; ?>
    <section class="customer-category" id="products"><div class="customer-section-title"><div><p class="customer-kicker">THE LITTLE GOOD THINGS</p><h2>Shop <em>products.</em></h2></div><p>One-off pieces and custom favourites, made to bring a little more joy to your everyday.</p></div><div class="customer-item-grid"><?php if (!$products): ?><p class="customer-empty">New products are on their way.</p><?php else: foreach ($products as $item) $renderItem($item); endif; ?></div></section>
